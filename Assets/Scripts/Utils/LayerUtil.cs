...
public class LayerUtil {
	...
	public static GameObject createLayer (string shaderName, string name, Vector3 position, Transform parentT)
	{
		...
		go.name=name;
#if UNITY_EDITOR
		go.transform.GetComponent<Renderer>().sharedMaterial=MaterialUtil.getMaterial(go.name,shaderName);				
#endif
	...
	public static GameObject createBufferLayer (string shaderName, string name, Vector3 position, Transform parentT){		
		...
		go.transform.GetComponent<Renderer>().sharedMaterial.SetTexture("_MainTex",texture);	
		return go;		
	}
