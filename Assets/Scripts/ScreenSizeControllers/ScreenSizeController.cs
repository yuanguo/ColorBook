...
public class ScreenSizeController : MonoBehaviour {
	...
	void setResolutionToActualSizeAndSendEvent () {				
		//Debug2LogDebug("setResolutionToActualSizeAndSendEvent current resolution	" + Screen.currentResolution);
		...
		if (Screen.width < PropertiesSingleton.instance.screen.minWidth || Screen.height < PropertiesSingleton.instance.screen.minHeight){
			...
			if (PropertiesSingleton.instance.m_textLog != null)
			{
				PropertiesSingleton.instance.m_textLog.text += ("\n Screen.width < PropertiesSingleton else");
			}
			...
			if(Screen.fullScreen){
				...
				//Debug2LogDebug("trying to change to full screen");	
	...
	void scaleScreenAndSendEvent () {
		//Debug2LogDebug("scaleScreenAndSendEvent ");
		float ratio;
//		Resolution maxRes= Screen.resolutions[Screen.resolutions.Length-1];
	...
	void sendEventOnResolutionChange(IntVector2 resolution, Vector2 scale){
		//Debug2LogDebug("sendEventOnResolutionChange  resolution = "+resolution.ToString()+"   scale ="+scale);
